...
ALLOWED_ORIGINS = os.getenv("ALLOWED_ORIGINS", "https://orangmods-api.onrender.com").split(",")
ALLOWED_HOSTS = os.getenv("ALLOWED_HOSTS", "orangmods-api.onrender.com,localhost").split(",")
...
# Синхронная версия для фоновых задач
@contextmanager
def get_db_sync():
...
async def init_database_async():
    async with get_db_async() as conn:
        ...
        # Получаем ID админа для создания тестовых ключей
        ...
        # Создание тестовых ключей (только если таблица пуста)
...
def format_key_row(row) -> dict:
    """Форматирует строку из БД в словарь ключа"""
    key_dict = {
        "id": row[0],
        "key_value": row[1],
        "name": row[2],
...
# ============================================================
# ПОЛНЫЙ HTML КОД АДМИНКИ (БЕЗ СОКРАЩЕНИЙ)
# ============================================================
...
<body>
    ...
    <div class="container">
        ...
        <div id="loginForm">
            <div class="field">
                <label>👤 Логин</label>
                <input type="text" id="loginInput" placeholder="Введите логин" value="Ad09oLq@Gmail.yandex" autocomplete="username" />
            </div>
            <div class="field">
                <label>🔑 Пароль</label>
                <input type="password" id="passInput" placeholder="Введите пароль" value="pa9w9diqllOoeje" autocomplete="current-password" />
            </div>

...
async def cleanup_expired_activations():
    try:
        ...
        try:
            ...
            """)
            logger.info("Cleaned up expired keys")
...
# ============================================================
# API - АКТИВАЦИЯ (ИСПРАВЛЕННАЯ — ИНДИВИДУАЛЬНЫЙ ТАЙМЕР)
# ============================================================
...
async def activate_key(activation: ActivationRequest, request: Request):
    ...
    try:
        ...
        if key_status not in ('active', 'waiting'):
        ...
        if existing:
            ...
            await conn.execute("""
                ...
                SET is_active = true, activated_at = NOW(), expires_at = $3
            """, key_id, activation.device_id, expires_at)
        
        # Обновляем used_devices (если устройство новое)
        if not existing:
            await conn.execute("""
                UPDATE keys 
            ...
            # Проверяем, активны ли другие устройства
            ...
            if active_devices > 0:
                await conn.execute("""
                    UPDATE keys SET status = 'active' WHERE id = $1
                """, key_id)

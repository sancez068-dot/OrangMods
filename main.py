...
async def init_database_async():
    async with get_db_async() as conn:
        ...
        # Добавляем дефолтные настройки (НОВЫЕ)
        ...
        if key_status == 'active' and force:
            ...
            for device_row in devices:
                ...
                """, (
                    device_id,
                    "⚠️ Ключ удален",
                    "Ваш ключ был удален администратором. Перезапустите приложение.",
...
# ============================================================
# API - ИНФОРМАЦИЯ О КЛЮЧЕ (НОВЫЙ ЭНДПОИНТ ДЛЯ APK)
# ============================================================

@app.get("/api/infokey/{key_code}/{device_id}")
async def get_key_info(key_code: str, device_id: str):
    """Получить информацию о ключе для устройства (обновление каждую секунду)"""
    async with get_db_async() as conn:
        cursor = await conn.cursor()
        
        # Проверяем ключ
        await cursor.execute("""
            SELECT k.id, k.max_percent, k.status, k.first_activation, 
                   k.type, k.duration, a.expires_at, a.is_active
            FROM keys k
        ...
        if expires_dt < now:
            ...
            # Обновляем статус ключа если все активации истекли
            await cursor.execute("""

...
async def init_database():
    ...
    try:
        ...
        # Таблица рекламы (создаем с полными полями)
        await conn.execute("""
        ...
        # ✅ МИГРАЦИЯ: Добавляем колонки если они отсутствуют
        try:
            # Проверяем существует ли колонка version
            col_check = await conn.fetchrow("""
                SELECT column_name 
                FROM information_schema.columns 
                WHERE table_name = 'advertisements' AND column_name = 'version'
            """)
            if not col_check:
                await conn.execute("ALTER TABLE advertisements ADD COLUMN version TEXT")
                logger.info("Added column 'version' to advertisements")
        except Exception as e:
            logger.warning(f"Could not add version column: {e}")
            
        try:
            col_check = await conn.fetchrow("""
                SELECT column_name 
                FROM information_schema.columns 
                WHERE table_name = 'advertisements' AND column_name = 'platform'
            """)
            if not col_check:
                await conn.execute("ALTER TABLE advertisements ADD COLUMN platform TEXT DEFAULT 'all'")
                logger.info("Added column 'platform' to advertisements")
        except Exception as e:
...
# ============================================================
# API - РЕКЛАМА (С ПОДДЕРЖКОЙ ВЕРСИЙ)
# ============================================================

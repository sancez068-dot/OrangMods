...
# Кеш для infokey (TTL - время жизни в секундах)
# Храним результаты на 30 секунд, чтобы снизить нагрузку на БД
INFO_CACHE = TTLCache(maxsize=50000, ttl=30)  # 50,000 устройств в кеше
CACHE_HITS = 0
CACHE_MISSES = 0
                ...
                if platform:
                    async with get_db_async() as conn:
                        ...
                        await cursor.execute(
                            "SELECT platform FROM devices WHERE device_id = ? AND is_active = 1",
                            (device_id,)
                        )
...
async def init_database_async():
    async with get_db_async() as conn:
        ...
        """)
        await cursor.execute("CREATE INDEX IF NOT EXISTS idx_devices_device_id ON devices(device_id)")
        await cursor.execute("CREATE INDEX IF NOT EXISTS idx_devices_push_token ON devices(push_token)")
        ...
        """)
        await cursor.execute("CREATE INDEX IF NOT EXISTS idx_device_notifications_device ON device_notifications(device_id)")
        await cursor.execute("CREATE INDEX IF NOT EXISTS idx_device_notifications_read ON device_notifications(is_read)")
        ...
        """)
        await cursor.execute("CREATE INDEX IF NOT EXISTS idx_settings_key ON settings(key)")
        ...
        await cursor.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('telegram_username', 'SofterTeamBot')")
        await cursor.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('app_name', 'OrangMods')")
        await cursor.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('support_email', 'support@orangmods.com')")
        
        # Создание тестовых ключей (только если нет ключей)
        ...
        await cursor.execute("SELECT COUNT(*) FROM advertisements")
        row = await cursor.fetchone()
        if row and row[0] == 0:
            await cursor.execute("""
                INSERT INTO advertisements (html, is_closable, is_active)
                VALUES (?, ?, ?)
            """, ('<div style="padding:20px;background:linear-gradient(135deg,#ff7300,#ffb300);border-radius:12px;color:#fff;text-align:center;font-size:18px;">🍊 OrangMods - Ваш лучший выбор!</div>', 1, 1))
...
async def delete_key(
    ...
    async with get_db_async() as conn:
        ...
        await cursor.execute(
            "SELECT key_value, status FROM keys WHERE id = ?", 
            (key_id,)
        )
        row = await cursor.fetchone()
        if not row:
        ...
        if key_status == 'active' and force:
            ...
            for device_row in devices:
                ...
                """, (
                    device_id,
                    "⚠️ Ключ удален",
                    "Ваш ключ был удален администратором. Доступ будет сброшен.",
        ...
        log_action(
            current_admin["id"], 
            "delete_key", 
            f"Deleted key: {key_value} (force={force})", 
            client_ip
        )
...
# ============================================================
# API - ИНФОРМАЦИЯ О КЛЮЧЕ (С КЕШИРОВАНИЕМ)
# ============================================================

@app.get("/api/infokey/{key_code}/{device_id}")
async def get_key_info(key_code: str, device_id: str):
    """Получить информацию о ключе для устройства (с кешированием)"""
    ...
    if cache_key in INFO_CACHE:
        ...
        # Если в кеше есть результат и он валидный - возвращаем
        if cached_result.get('valid') and cached_result.get('hours_remaining', 0) > 0:
            logger.debug(f"Cache hit for {cache_key} (hits: {CACHE_HITS})")
            return cached_result
    
    CACHE_MISSES += 1
    logger.debug(f"Cache miss for {cache_key} (misses: {CACHE_MISSES})")
    ...
    async with get_db_async() as conn:
        ...
        await cursor.execute("""
            ...
            FROM keys k
            LEFT JOIN activations a ON k.id = a.key_id AND a.device_id = ?
        ...
        # Проверяем истекла ли активация
        try:
        ...
        if expires_dt < now:
            ...
            await cursor.execute("""
                ...
                WHERE id = ? AND NOT EXISTS (
                    ...
                    WHERE key_id = keys.id AND is_active = 1
                )
            """, (key_id,))
        ...
        # Кешируем только если ключ валидный и осталось больше 5 минут
...
async def update_settings(
    ...
    async with get_db_async() as conn:
        ...
        client_ip = request.client.host if request else "unknown"
        log_action(current_admin["id"], "update_settings", f"Updated settings: {list(settings_data.keys())}", client_ip)
...
async def create_notification(notif_data: NotificationCreate, current_admin: dict = Depends(get_current_admin), request: Request = None):
    async with get_db_async() as conn:
        ...
        # Отправляем через WebSocket всем устройствам
...
async def register_device(device_data: DeviceRegister, request: Request):
    async with get_db_async() as conn:
        ...
        if existing:
            await cursor.execute("""
                UPDATE devices 
                SET platform = ?, push_token = ?, last_active = CURRENT_TIMESTAMP, is_active = 1
                WHERE device_id = ?
...
async def send_device_notification(
    ...
    async with get_db_async() as conn:
        ...
        await cursor.execute(
            "SELECT platform, push_token FROM devices WHERE device_id = ? AND is_active = 1",
...
async def broadcast_notification(
    ...
    async with get_db_async() as conn:
        ...
        query = "SELECT device_id, push_token FROM devices WHERE is_active = 1"
        params = []
        ...
        # Сохраняем уведомления для ВСЕХ устройств в БД
        saved_count = 0
        for row in rows:
            await cursor.execute("""
            ...
            """, (
                ...
                broadcast.title,
                broadcast.body,
                json.dumps(broadcast.data) if broadcast.data else None
            ))
            saved_count += 1
        
        # Отправляем через WebSocket только активным подключениям
...
async def mark_all_read(device_id: str):
    async with get_db_async() as conn:
        ...
        return {"success": True, "read_count": cursor.rowcount}
...
# ============================================================
# API - СТАТИСТИКА КЕША (ДЛЯ МОНИТОРИНГА)
# ============================================================
...
async def get_ws_status(device_id: str, current_admin: dict = Depends(get_current_admin)):
    """Проверить статус WebSocket соединения устройства"""
    is_connected = device_id in manager.active_connections
    
    async with get_db_async() as conn:
        cursor = await conn.cursor()
        await cursor.execute(
            "SELECT platform, is_active, last_active FROM devices WHERE device_id = ?",
            (device_id,)
        )
        row = await cursor.fetchone()
...
async def websocket_device(websocket: WebSocket, device_id: str):
    async with get_db_async() as conn:
        ...
        await cursor.execute(
            "UPDATE devices SET last_active = CURRENT_TIMESTAMP WHERE device_id = ?",
            (device_id,)
        )

...
async def init_database():
    ...
    try:
        ...
        if row and row[0] == 0:
            test_keys = [
                ("OrangMods-7DAY-XYZ123", "Тестовый ключ 1", "DAY", 7, 5, 70, "waiting"),
                ("OrangMods-30DAY-ABC456", "Тестовый ключ 2", "DAY", 30, 10, 80, "waiting"),
                ("OrangMods-12HOUR-DEF789", "Тестовый ключ 3", "HOUR", 12, 3, 90, "waiting"),
...
async def lifespan(app: FastAPI):
    ...
    scheduler.add_job(
        ...
        trigger=IntervalTrigger(minutes=5),  # Увеличили частоту для более точного контроля
...
async def calculate_device_time_left(key_id: int) -> dict:
    """
    Рассчитывает минимальный остаток времени среди всех активных устройств для ключа
    """
    conn = await get_db_connection()
    try:
        # Находим самую раннюю активную активацию
...
async def format_key_row(row) -> dict:
    ...
    # Для активных ключей считаем индивидуальный остаток
...
async def cleanup_expired_activations():
    try:
        conn = await get_db_connection()
        try:
            # 1. Деактивируем истекшие активации
            ...
            # 2. Обновляем used_devices (реальное количество активных устройств)
            await conn.execute("""
                UPDATE keys 
                SET used_devices = (
                    SELECT COUNT(*) 
            ...
            # 3. Обновляем статус ключей
            # Если нет активных активаций, но есть использованные слоты - ставим waiting
            await conn.execute("""
                UPDATE keys 
                SET status = 'waiting' 
                WHERE status = 'active' 
            ...
            # 4. Обновляем статус FULL если достигнут лимит (учитываем ВСЕ активации)
            await conn.execute("""
                UPDATE keys 
                SET status = 'full' 
            ...
            # 5. Если ключ был full и освободились слоты - возвращаем active/waiting
            await conn.execute("""
                UPDATE keys 
                SET status = CASE 
                    WHEN EXISTS (
    ...
    try:
        ...
        for row in rows:
            ...
            if key_dict['status'] == 'deleted':
                ...
                key_dict['status'] = 'waiting'  # Все слоты заняты, но никто не активен
...
async def delete_key(
    ...
    try:
        ...
        # Если ключ активен и включен force, деактивируем все устройства
        if (key_status == 'active' or key_status == 'full') and force:
            # Получаем все активные устройства для этого ключа
            devices = await conn.fetch(
                "SELECT device_id FROM activations WHERE key_id = $1 AND is_active = true",
                key_id
            )
            
            # Деактивируем все активации для этого ключа
            await conn.execute(
                "UPDATE activations SET is_active = false WHERE key_id = $1",
                key_id
            )
            
            # Отправляем уведомления и очищаем кеш для каждого устройства
            ...
            # Обновляем статус ключа на 'deleted'
            await conn.execute(
            ...
            # Если ключ активен и force=false, предупреждаем
...
# ============================================================
# API - АКТИВАЦИЯ (ИНДИВИДУАЛЬНЫЙ ТАЙМЕР)
# ============================================================
...
async def activate_key(activation: ActivationRequest, request: Request):
    ...
    try:
        ...
        if existing:
            # Устройство уже использовало ключ - реактивируем с новым таймером
            await conn.execute("""
                UPDATE activations 
        ...
        # Если достигнут лимит — full, иначе active
...
async def check_key(key_code: str, device_id: str):
    ...
    try:
        ...
        # Проверяем активна ли активация
...
async def get_key_info(key_code: str, device_id: str):
    ...
    try:
        ...
        if expires_dt < datetime.now():
            ...
            # Проверяем, есть ли еще активные устройства
            active_count = await conn.fetchval(
                "SELECT COUNT(*) FROM activations WHERE key_id = $1 AND is_active = true",
                result['id']
            )
            if active_count == 0:
                # Если нет активных, но есть использованные слоты - ставим waiting
